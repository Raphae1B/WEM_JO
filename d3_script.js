...
function drawHeatmap({ data, sortedDisciplines }) {
        ...
        .on("click", function(event, year) {
            ...
            d3.selectAll(".y-axis text")
                .style("font-weight", function(d) {
                    return d3.selectAll("rect.data-rect").filter(dd => dd.year === year && dd.discipline === d).size() > 0 ? "bold" : "normal";
                })
                .style("opacity", function(d) {
                    return d3.selectAll("rect.data-rect").filter(dd => dd.year === year && dd.discipline === d).size() > 0 ? 1 : 0.6;  // Opacité réduite pour les disciplines non sélectionnées
                });
    ...
    svg.selectAll("myRects")
        ...
        .on("mouseover", function(event, d) {
            tooltip.html(`Discipline: ${d.discipline}<br>Year: ${d.year}`)
...
function drawMedalChart(data, year = null) {
    ...
    const layerGroups = svg.selectAll(".layer")
        ...
        .on("click", function(event, d) {
            tooltip.html(`Gold: ${d.data.Gold}<br>Silver: ${d.data.Silver}<br>Bronze: ${d.data.Bronze}<br>Total: ${d.data.Total}`)
                .style("visibility", "visible")
                .style("left", (event.pageX + 10) + "px")
